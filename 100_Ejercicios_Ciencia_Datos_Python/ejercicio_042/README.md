# Ejercicio 042 — Margen con catálogo de costes

[Índice](../README.md#indice-de-ejercicios) · [Anterior](../ejercicio_041/README.md) · [Siguiente](../ejercicio_043/README.md)

### Contexto

Una librería dispone de costes por producto. Sector: **comercio**. Todos los registros son sintéticos; las entidades y situaciones no describen organizaciones reales.

### Problema

El producto con más ingresos se considera el más rentable. El encargo requiere comprobar esa lectura con evidencia, sin aceptar como conclusión lo que plantea la situación.

### Pregunta principal

¿Cómo cambia el orden de productos al estimar margen bruto?
...
- [registros](datos/registros.csv): **1,224 registros**, 8 campos. Fuente principal: una operación de un producto.
...
Evalúa ausencias, repeticiones, formatos, rangos y coherencia entre campos. En este contexto debes considerar: **El coste de referencia es fijo para el período y excluye gastos indirectos.** Para cada problema encontrado, registra evidencia, decisión —conservar, marcar, corregir, investigar, imputar o excluir—, justificación y efecto sobre la población. No borres nulos ni extremos por una regla universal. Mantén los originales y contrasta el resultado bajo una decisión alternativa razonable.

### Requisitos del análisis

Relaciona catálogo y operaciones; compara ingreso y margen con cobertura de costes; separa descuentos y anulaciones.
...
**Joins muchos a uno; margen; escenarios.** Estudia qué pregunta responde cada concepto, bajo qué supuestos y qué puede ocultar. Relaciona su significado con la unidad de observación del paquete antes de elegir una función de biblioteca.
...
Convierte el encargo en evidencia sobre **margen con catálogo de costes**. Presenta la comparación principal y una alternativa que pueda cuestionarla; indica el tamaño de cada grupo o ventana y explica qué cambió al preparar los datos. ¿El orden por unidades coincide con el orden por importe? Expón al menos una explicación rival antes de redactar una recomendación.

### Visualización

Diseña una figura que permita responder «¿Cómo cambia el orden de productos al estimar margen bruto?». Elige la representación según tipo de variable y audiencia. Incluye población, unidades, período y denominadores; añade incertidumbre cuando sea parte de tu metodología. Escribe una observación y una afirmación que esa figura no demuestra.
...
- El coste de referencia es fijo para el período y excluye gastos indirectos.
...
Aceptar la afirmación del problema sin comprobarla; confundir filas con entidades independientes; cambiar un denominador sin documentarlo; sumar o promediar sin revisar unidades; elegir el gráfico o la prueba que más favorezca una conclusión. En particular, explica cómo evitarías este riesgo: **El coste de referencia es fijo para el período y excluye gastos indirectos.**
...
Antes de consultar, escribe una pregunta concreta sobre **joins muchos a uno**. Después registra el apartado leído, su supuesto principal y cómo lo aplicaste. Consulta también la [guía de investigación](../RECURSOS.md); no busques un notebook resuelto del encargo.
...
¿Qué observaciones, variables o mecanismos de selección faltan para sostener una conclusión más fuerte? ¿Qué parte de la pregunta queda sin responder? Revisa especialmente: **El coste de referencia es fijo para el período y excluye gastos indirectos.** Los datos sintéticos sirven para entrenar razonamiento; sus patrones no estiman parámetros de una población real.
...
Simula un incremento del 15% en costes y evalúa estabilidad de la prioridad. Conserva la primera versión, cambia solo las condiciones declaradas y compara evidencia y conclusión. Documenta qué componentes de tu código pudiste reutilizar y qué supuesto dejó de ser válido.

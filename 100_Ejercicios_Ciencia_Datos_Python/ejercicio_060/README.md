# Ejercicio 060 — Resultado de un ensayo A/B

[Índice](../README.md#indice-de-ejercicios) · [Anterior](../ejercicio_059/README.md) · [Siguiente](../ejercicio_061/README.md)

### Contexto

Un producto evalúa una variante de bienvenida. Sector: **tecnología y marketing**. Todos los registros son sintéticos; las entidades y situaciones no describen organizaciones reales.

### Problema

La dirección pide elegir la versión con mayor conversión puntual. El encargo requiere comprobar esa lectura con evidencia, sin aceptar como conclusión lo que plantea la situación.

### Pregunta principal

¿La diferencia estimada justifica la decisión según magnitud e incertidumbre?
...
- [registros](datos/registros.json): **1,400 registros**, 9 campos. Fuente principal: un usuario asignado una vez.
- [asignacion](datos/asignacion.csv): **1,400 registros**, 3 campos. Registro de asignación. Conservar usuarios aunque no se expongan al mensaje.
...
| Campo | Significado | Tipo esperado |
| --- | --- | --- |
| `registro_id` | Usuario asignado | texto |
| `grupo_asignado` | A o B | categoría |
| `fecha_asignacion` | Fecha de asignación | fecha |
...
Evalúa ausencias, repeticiones, formatos, rangos y coherencia entre campos. En este contexto debes considerar: **Aleatorización no corrige pérdida diferencial de seguimiento; significancia no equivale a importancia.** Para cada problema encontrado, registra evidencia, decisión —conservar, marcar, corregir, investigar, imputar o excluir—, justificación y efecto sobre la población. No borres nulos ni extremos por una regla universal. Mantén los originales y contrasta el resultado bajo una decisión alternativa razonable.

### Requisitos del análisis

Usa intención de tratar, diferencia absoluta y relativa, intervalo y contraste; declara manejo del seguimiento faltante y una mejora relevante de 2 puntos.
...
**Ensayos; efecto; potencia; incertidumbre.** Estudia qué pregunta responde cada concepto, bajo qué supuestos y qué puede ocultar. Relaciona su significado con la unidad de observación del paquete antes de elegir una función de biblioteca.
...
Convierte el encargo en evidencia sobre **resultado de un ensayo a/b**. Presenta la comparación principal y una alternativa que pueda cuestionarla; indica el tamaño de cada grupo o ventana y explica qué cambió al preparar los datos. ¿La exposición y el seguimiento son comparables entre grupos asignados? Expón al menos una explicación rival antes de redactar una recomendación.

### Visualización

Diseña una figura que permita responder «¿La diferencia estimada justifica la decisión según magnitud e incertidumbre?». Elige la representación según tipo de variable y audiencia. Incluye población, unidades, período y denominadores; añade incertidumbre cuando sea parte de tu metodología. Escribe una observación y una afirmación que esa figura no demuestra.
...
- Aleatorización no corrige pérdida diferencial de seguimiento; significancia no equivale a importancia.
...
Aceptar la afirmación del problema sin comprobarla; confundir filas con entidades independientes; cambiar un denominador sin documentarlo; sumar o promediar sin revisar unidades; elegir el gráfico o la prueba que más favorezca una conclusión. En particular, explica cómo evitarías este riesgo: **Aleatorización no corrige pérdida diferencial de seguimiento; significancia no equivale a importancia.**
...
Antes de consultar, escribe una pregunta concreta sobre **ensayos**. Después registra el apartado leído, su supuesto principal y cómo lo aplicaste. Consulta también la [guía de investigación](../RECURSOS.md); no busques un notebook resuelto del encargo.
...
¿Qué observaciones, variables o mecanismos de selección faltan para sostener una conclusión más fuerte? ¿Qué parte de la pregunta queda sin responder? Revisa especialmente: **Aleatorización no corrige pérdida diferencial de seguimiento; significancia no equivale a importancia.** Los datos sintéticos sirven para entrenar razonamiento; sus patrones no estiman parámetros de una población real.
...
Estima por simulación qué tamaño muestral permitiría detectar una diferencia de 2 puntos bajo supuestos explícitos. Conserva la primera versión, cambia solo las condiciones declaradas y compara evidencia y conclusión. Documenta qué componentes de tu código pudiste reutilizar y qué supuesto dejó de ser válido.

# Ejercicio 071 — Llegadas y resolución de solicitudes

[Índice](../README.md#indice-de-ejercicios) · [Anterior](../ejercicio_070/README.md) · [Siguiente](../ejercicio_072/README.md)

### Contexto

Soporte planifica turnos semanales. Sector: **soporte informático**. Todos los registros son sintéticos; las entidades y situaciones no describen organizaciones reales.

### Problema

Los cierres se presentan como si midieran demanda entrante. El encargo requiere comprobar esa lectura con evidencia, sin aceptar como conclusión lo que plantea la situación.

### Pregunta principal

¿Cómo evolucionan llegadas, cierres y acumulación de tickets?
...
- [registros](datos/registros.csv): **2,107 registros**, 8 campos. Fuente principal: un ticket.
- [eventos](datos/eventos.csv): **4,318 registros**, 4 campos. Historial de aperturas, cierres y reaperturas. No hay backlog anterior a la primera fecha.
...
Evalúa ausencias, repeticiones, formatos, rangos y coherencia entre campos. En este contexto debes considerar: **Un cierre duplicado no reduce dos veces el backlog.** Para cada problema encontrado, registra evidencia, decisión —conservar, marcar, corregir, investigar, imputar o excluir—, justificación y efecto sobre la población. No borres nulos ni extremos por una regla universal. Mantén los originales y contrasta el resultado bajo una decisión alternativa razonable.

### Requisitos del análisis

Reconstruye flujos desde eventos, concilia stock inicial más entradas menos salidas con stock final y separa reaperturas.
...
**Stocks y flujos; conservación; series.** Estudia qué pregunta responde cada concepto, bajo qué supuestos y qué puede ocultar. Relaciona su significado con la unidad de observación del paquete antes de elegir una función de biblioteca.
...
Convierte el encargo en evidencia sobre **llegadas y resolución de solicitudes**. Presenta la comparación principal y una alternativa que pueda cuestionarla; indica el tamaño de cada grupo o ventana y explica qué cambió al preparar los datos. ¿Qué cambia al distinguir prioridad y estado al corte? Expón al menos una explicación rival antes de redactar una recomendación.

### Visualización

Diseña una figura que permita responder «¿Cómo evolucionan llegadas, cierres y acumulación de tickets?». Elige la representación según tipo de variable y audiencia. Incluye población, unidades, período y denominadores; añade incertidumbre cuando sea parte de tu metodología. Escribe una observación y una afirmación que esa figura no demuestra.
...
- Un cierre duplicado no reduce dos veces el backlog.
...
Aceptar la afirmación del problema sin comprobarla; confundir filas con entidades independientes; cambiar un denominador sin documentarlo; sumar o promediar sin revisar unidades; elegir el gráfico o la prueba que más favorezca una conclusión. En particular, explica cómo evitarías este riesgo: **Un cierre duplicado no reduce dos veces el backlog.**
...
Antes de consultar, escribe una pregunta concreta sobre **stocks y flujos**. Después registra el apartado leído, su supuesto principal y cómo lo aplicaste. Consulta también la [guía de investigación](../RECURSOS.md); no busques un notebook resuelto del encargo.
...
¿Qué observaciones, variables o mecanismos de selección faltan para sostener una conclusión más fuerte? ¿Qué parte de la pregunta queda sin responder? Revisa especialmente: **Un cierre duplicado no reduce dos veces el backlog.** Los datos sintéticos sirven para entrenar razonamiento; sus patrones no estiman parámetros de una población real.
...
Simula una capacidad semanal fija sin confundir simulación con resultado observado. Conserva la primera versión, cambia solo las condiciones declaradas y compara evidencia y conclusión. Documenta qué componentes de tu código pudiste reutilizar y qué supuesto dejó de ser válido.

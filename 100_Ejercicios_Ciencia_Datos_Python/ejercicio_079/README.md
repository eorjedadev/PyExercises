# Ejercicio 079 — Fallos poco frecuentes

[Índice](../README.md#indice-de-ejercicios) · [Anterior](../ejercicio_078/README.md) · [Siguiente](../ejercicio_080/README.md)

### Contexto

Mantenimiento evalúa alertas de fallo a siete días. Sector: **industria**. Todos los registros son sintéticos; las entidades y situaciones no describen organizaciones reales.

### Problema

Una accuracy alta puede obtenerse sin detectar fallos. El encargo requiere comprobar esa lectura con evidencia, sin aceptar como conclusión lo que plantea la situación.

### Pregunta principal

¿Qué modelo mejora la detección respecto a predecir siempre no fallo?
...
- [registros](datos/registros.csv): **3,010 registros**, 12 campos. Fuente principal: un lote producido.
...
Evalúa ausencias, repeticiones, formatos, rangos y coherencia entre campos. En este contexto debes considerar: **Coste de reparación se conoce tras el fallo; etiquetas con horizontes solapados pueden contaminar la evaluación.** Para cada problema encontrado, registra evidencia, decisión —conservar, marcar, corregir, investigar, imputar o excluir—, justificación y efecto sobre la población. No borres nulos ni extremos por una regla universal. Mantén los originales y contrasta el resultado bajo una decisión alternativa razonable.

### Requisitos del análisis

Define corte e información previa, separa por tiempo con separación del horizonte, compara recall, precision y F1, y analiza falsos negativos.
...
**Desbalance; métricas; costes de decisión.** Estudia qué pregunta responde cada concepto, bajo qué supuestos y qué puede ocultar. Relaciona su significado con la unidad de observación del paquete antes de elegir una función de biblioteca.
...
Convierte el encargo en evidencia sobre **fallos poco frecuentes**. Presenta la comparación principal y una alternativa que pueda cuestionarla; indica el tamaño de cada grupo o ventana y explica qué cambió al preparar los datos. ¿Qué cambia al comparar turnos y tamaños de lote? Expón al menos una explicación rival antes de redactar una recomendación.

### Visualización

Diseña una figura que permita responder «¿Qué modelo mejora la detección respecto a predecir siempre no fallo?». Elige la representación según tipo de variable y audiencia. Incluye población, unidades, período y denominadores; añade incertidumbre cuando sea parte de tu metodología. Escribe una observación y una afirmación que esa figura no demuestra.
...
- Coste de reparación se conoce tras el fallo; etiquetas con horizontes solapados pueden contaminar la evaluación.
...
Aceptar la afirmación del problema sin comprobarla; confundir filas con entidades independientes; cambiar un denominador sin documentarlo; sumar o promediar sin revisar unidades; elegir el gráfico o la prueba que más favorezca una conclusión. En particular, explica cómo evitarías este riesgo: **Coste de reparación se conoce tras el fallo; etiquetas con horizontes solapados pueden contaminar la evaluación.**
...
Antes de consultar, escribe una pregunta concreta sobre **desbalance**. Después registra el apartado leído, su supuesto principal y cómo lo aplicaste. Consulta también la [guía de investigación](../RECURSOS.md); no busques un notebook resuelto del encargo.
...
¿Qué observaciones, variables o mecanismos de selección faltan para sostener una conclusión más fuerte? ¿Qué parte de la pregunta queda sin responder? Revisa especialmente: **Coste de reparación se conoce tras el fallo; etiquetas con horizontes solapados pueden contaminar la evaluación.** Los datos sintéticos sirven para entrenar razonamiento; sus patrones no estiman parámetros de una población real.
...
Introduce costes hipotéticos de falsa alarma y fallo omitido y justifica umbral. Conserva la primera versión, cambia solo las condiciones declaradas y compara evidencia y conclusión. Documenta qué componentes de tu código pudiste reutilizar y qué supuesto dejó de ser válido.

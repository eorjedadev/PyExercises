# Ejercicio 080 — Cancelación antes de la llegada

[Índice](../README.md#indice-de-ejercicios) · [Anterior](../ejercicio_079/README.md) · [Siguiente](../ejercicio_081/README.md)

### Contexto

Una cadena planifica reservas. Sector: **turismo**. Todos los registros son sintéticos; las entidades y situaciones no describen organizaciones reales.

### Problema

Se mezclan datos disponibles al reservar y al cerrar la estancia. El encargo requiere comprobar esa lectura con evidencia, sin aceptar como conclusión lo que plantea la situación.

### Pregunta principal

¿Qué predicción de cancelación puede producirse al registrar una reserva?
...
- [registros](datos/registros.csv): **3,000 registros**, 9 campos. Fuente principal: una reserva.
...
Evalúa ausencias, repeticiones, formatos, rangos y coherencia entre campos. En este contexto debes considerar: **Datos posteriores a la decisión invalidan la evaluación; canal nuevo puede no existir en entrenamiento.** Para cada problema encontrado, registra evidencia, decisión —conservar, marcar, corregir, investigar, imputar o excluir—, justificación y efecto sobre la población. No borres nulos ni extremos por una regla universal. Mantén los originales y contrasta el resultado bajo una decisión alternativa razonable.

### Requisitos del análisis

Define momento de predicción, separa por reserva temporalmente y compara baseline y clasificación con métricas justificadas.
...
**Momento de predicción; categorías; generalización.** Estudia qué pregunta responde cada concepto, bajo qué supuestos y qué puede ocultar. Relaciona su significado con la unidad de observación del paquete antes de elegir una función de biblioteca.
...
Convierte el encargo en evidencia sobre **cancelación antes de la llegada**. Presenta la comparación principal y una alternativa que pueda cuestionarla; indica el tamaño de cada grupo o ventana y explica qué cambió al preparar los datos. ¿Los canales tienen la misma anticipación y composición de hoteles? Expón al menos una explicación rival antes de redactar una recomendación.

### Visualización

Diseña una figura que permita responder «¿Qué predicción de cancelación puede producirse al registrar una reserva?». Elige la representación según tipo de variable y audiencia. Incluye población, unidades, período y denominadores; añade incertidumbre cuando sea parte de tu metodología. Escribe una observación y una afirmación que esa figura no demuestra.
...
- Datos posteriores a la decisión invalidan la evaluación; canal nuevo puede no existir en entrenamiento.
...
Aceptar la afirmación del problema sin comprobarla; confundir filas con entidades independientes; cambiar un denominador sin documentarlo; sumar o promediar sin revisar unidades; elegir el gráfico o la prueba que más favorezca una conclusión. En particular, explica cómo evitarías este riesgo: **Datos posteriores a la decisión invalidan la evaluación; canal nuevo puede no existir en entrenamiento.**
...
Antes de consultar, escribe una pregunta concreta sobre **momento de predicción**. Después registra el apartado leído, su supuesto principal y cómo lo aplicaste. Consulta también la [guía de investigación](../RECURSOS.md); no busques un notebook resuelto del encargo.
...
¿Qué observaciones, variables o mecanismos de selección faltan para sostener una conclusión más fuerte? ¿Qué parte de la pregunta queda sin responder? Revisa especialmente: **Datos posteriores a la decisión invalidan la evaluación; canal nuevo puede no existir en entrenamiento.** Los datos sintéticos sirven para entrenar razonamiento; sus patrones no estiman parámetros de una población real.
...
Evalúa por hotel y canal, sin ajustar el modelo con el conjunto final. Conserva la primera versión, cambia solo las condiciones declaradas y compara evidencia y conclusión. Documenta qué componentes de tu código pudiste reutilizar y qué supuesto dejó de ser válido.

# Ejercicio 086 — Pronóstico y validación móvil

[Índice](../README.md#indice-de-ejercicios) · [Anterior](../ejercicio_085/README.md) · [Siguiente](../ejercicio_087/README.md)

### Contexto

Un administrador compara pronósticos de consumo. Sector: **energía**. Todos los registros son sintéticos; las entidades y situaciones no describen organizaciones reales.

### Problema

Un único corte puede favorecer un período particular. El encargo requiere comprobar esa lectura con evidencia, sin aceptar como conclusión lo que plantea la situación.

### Pregunta principal

¿Qué método mantiene utilidad en varios orígenes de pronóstico?
...
Evalúa ausencias, repeticiones, formatos, rangos y coherencia entre campos. En este contexto debes considerar: **No entrenar con datos posteriores al origen; ventanas de evaluación solapadas requieren cautela.** Para cada problema encontrado, registra evidencia, decisión —conservar, marcar, corregir, investigar, imputar o excluir—, justificación y efecto sobre la población. No borres nulos ni extremos por una regla universal. Mantén los originales y contrasta el resultado bajo una decisión alternativa razonable.

### Requisitos del análisis

Diseña evaluación con origen móvil y horizonte de siete días, compara baseline estacional y modelo, e informa MAE y RMSE por edificio.
...
**Backtesting; horizonte; estacionalidad.** Estudia qué pregunta responde cada concepto, bajo qué supuestos y qué puede ocultar. Relaciona su significado con la unidad de observación del paquete antes de elegir una función de biblioteca.
...
Convierte el encargo en evidencia sobre **pronóstico y validación móvil**. Presenta la comparación principal y una alternativa que pueda cuestionarla; indica el tamaño de cada grupo o ventana y explica qué cambió al preparar los datos. ¿Qué cambia al considerar ocupación, clima y superficie? Expón al menos una explicación rival antes de redactar una recomendación.

### Visualización

Diseña una figura que permita responder «¿Qué método mantiene utilidad en varios orígenes de pronóstico?». Elige la representación según tipo de variable y audiencia. Incluye población, unidades, período y denominadores; añade incertidumbre cuando sea parte de tu metodología. Escribe una observación y una afirmación que esa figura no demuestra.
...
- No entrenar con datos posteriores al origen; ventanas de evaluación solapadas requieren cautela.
...
Aceptar la afirmación del problema sin comprobarla; confundir filas con entidades independientes; cambiar un denominador sin documentarlo; sumar o promediar sin revisar unidades; elegir el gráfico o la prueba que más favorezca una conclusión. En particular, explica cómo evitarías este riesgo: **No entrenar con datos posteriores al origen; ventanas de evaluación solapadas requieren cautela.**
...
Antes de consultar, escribe una pregunta concreta sobre **backtesting**. Después registra el apartado leído, su supuesto principal y cómo lo aplicaste. Consulta también la [guía de investigación](../RECURSOS.md); no busques un notebook resuelto del encargo.
...
¿Qué observaciones, variables o mecanismos de selección faltan para sostener una conclusión más fuerte? ¿Qué parte de la pregunta queda sin responder? Revisa especialmente: **No entrenar con datos posteriores al origen; ventanas de evaluación solapadas requieren cautela.** Los datos sintéticos sirven para entrenar razonamiento; sus patrones no estiman parámetros de una población real.
...
Evalúa horizonte de 30 días sin reutilizar futuras covariables observadas. Conserva la primera versión, cambia solo las condiciones declaradas y compara evidencia y conclusión. Documenta qué componentes de tu código pudiste reutilizar y qué supuesto dejó de ser válido.

# Ejercicio 096 — Diagnóstico de una red logística

[Índice](../README.md#indice-de-ejercicios) · [Anterior](../ejercicio_095/README.md) · [Siguiente](../ejercicio_097/README.md)

### Contexto

Un operador integra rutas, entregas y tarifas. Sector: **transporte y logística**. Todos los registros son sintéticos; las entidades y situaciones no describen organizaciones reales.

### Problema

Se quiere reducir coste manteniendo servicio. El encargo requiere comprobar esa lectura con evidencia, sin aceptar como conclusión lo que plantea la situación.

### Pregunta principal

¿Qué cambio operativo merece probarse y qué datos lo respaldan?
...
- [registros](datos/registros.csv): **4,515 registros**, 9 campos. Fuente principal: un envío.
...
- [rutas](datos/rutas.csv): **6 registros**, 3 campos. Catálogo de rutas; ventana no es duración prometida por envío.
...
| --- | --- | --- |
| `ruta` | Ruta | texto |
| `tipo` | Tipo de recorrido | categoría |
| `ventana_h` | Horas nominales de ventana operativa | entero |
...
Evalúa ausencias, repeticiones, formatos, rangos y coherencia entre campos. En este contexto debes considerar: **Rutas no se asignan al azar; tarifas omiten costes extraordinarios.** Para cada problema encontrado, registra evidencia, decisión —conservar, marcar, corregir, investigar, imputar o excluir—, justificación y efecto sobre la población. No borres nulos ni extremos por una regla universal. Mantén los originales y contrasta el resultado bajo una decisión alternativa razonable.

### Requisitos del análisis

Define frontera de servicio, compara rutas y transportistas, estudia colas y estacionalidad y evalúa sensibilidad del coste estimado.
...
**Compromisos; incertidumbre; decisión operacional.** Estudia qué pregunta responde cada concepto, bajo qué supuestos y qué puede ocultar. Relaciona su significado con la unidad de observación del paquete antes de elegir una función de biblioteca.
...
Convierte el encargo en evidencia sobre **diagnóstico de una red logística**. Presenta la comparación principal y una alternativa que pueda cuestionarla; indica el tamaño de cada grupo o ventana y explica qué cambió al preparar los datos. ¿El patrón se mantiene en rutas de distinta distancia y carga? Expón al menos una explicación rival antes de redactar una recomendación.

### Visualización

Diseña una figura que permita responder «¿Qué cambio operativo merece probarse y qué datos lo respaldan?». Elige la representación según tipo de variable y audiencia. Incluye población, unidades, período y denominadores; añade incertidumbre cuando sea parte de tu metodología. Escribe una observación y una afirmación que esa figura no demuestra.
...
- Rutas no se asignan al azar; tarifas omiten costes extraordinarios.
...
Aceptar la afirmación del problema sin comprobarla; confundir filas con entidades independientes; cambiar un denominador sin documentarlo; sumar o promediar sin revisar unidades; elegir el gráfico o la prueba que más favorezca una conclusión. En particular, explica cómo evitarías este riesgo: **Rutas no se asignan al azar; tarifas omiten costes extraordinarios.**
...
Antes de consultar, escribe una pregunta concreta sobre **compromisos**. Después registra el apartado leído, su supuesto principal y cómo lo aplicaste. Consulta también la [guía de investigación](../RECURSOS.md); no busques un notebook resuelto del encargo.
...
¿Qué observaciones, variables o mecanismos de selección faltan para sostener una conclusión más fuerte? ¿Qué parte de la pregunta queda sin responder? Revisa especialmente: **Rutas no se asignan al azar; tarifas omiten costes extraordinarios.** Los datos sintéticos sirven para entrenar razonamiento; sus patrones no estiman parámetros de una población real.
...
Impón un límite de aumento de retrasos de 2 puntos y revisa la recomendación. Conserva la primera versión, cambia solo las condiciones declaradas y compara evidencia y conclusión. Documenta qué componentes de tu código pudiste reutilizar y qué supuesto dejó de ser válido.

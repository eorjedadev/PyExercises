# Recursos del ejercicio 028

[Volver al enunciado](../ejercicio_028.md) · [Convenciones](../README.md#contratos-comunes)
...
### [datos/mensaje.bin](datos/mensaje.bin)

Mensaje de longitud cero del vector público Ed25519; es un archivo vacío intencional.

Contenido binario inerte; 0 bytes. No decodificar antes de una comprobación de integridad.

### [datos/alterado.bin](datos/alterado.bin)

Mensaje alterado: un byte adicional.

Contenido binario inerte; 1 bytes. No decodificar antes de una comprobación de integridad.

### [config/verificacion.json](config/verificacion.json)

Vector público de RFC8032 sección7.1 TEST1; no se entrega clave privada.

JSON: objeto; claves de primer nivel: public_key_hex, signature_hex, algorithm.

### [datos/clave_truncada.json](datos/clave_truncada.json)

Caso de formato incorrecto; no reemplaza la configuración válida.

JSON: objeto; claves de primer nivel: public_key_hex, signature_hex.

### [config/dependencia.json](config/dependencia.json)

Única dependencia externa de implementación obligatoria en estos ejercicios; el generador del material no requiere instalarla.

JSON: objeto; claves de primer nivel: package, documentation, purpose, installation.
...
Para implementar este ejercicio necesitas cryptography en un entorno virtual propio. Consulta la distribución oficial estable, registra la versión y comprueba primero el vector entregado. No implementes la primitiva ni reutilices las claves o nonces de estas muestras. El resto de la colección se puede trabajar con la biblioteca estándar.

# Ejercicio 005. Biblioteca personal con prestamos
...
Construir una solucion funcional para `Biblioteca personal con prestamos` usando una estructura ordenada. El programa debe tener una entrada de datos, una capa de procesamiento y una salida verificable. Evita resolverlo todo dentro de un solo bloque.
...
Tu solucion debe modelar una entidad principal llamada `ItemCatalogo` y una estructura que administre muchos elementos, por ejemplo `Catalogo`. No te quedes en imprimir datos: el programa debe recibir, validar, transformar y mostrar informacion util.

## Operaciones minimas

- registrar item.
- prestar o marcar visto.
- buscar por titulo.
...
- un item debe tener titulo.
- un prestamo debe registrar responsable.
- no se presta algo no disponible.
...
Piensa si tu solucion necesita funciones o metodos parecidos a estos: `agregar_item()`, `buscar_item()`, `marcar_estado()`, `filtrar_catalogo()`, `reporte_catalogo()`. No copies estos nombres sin pensar; usalos como pista para descubrir responsabilidades.

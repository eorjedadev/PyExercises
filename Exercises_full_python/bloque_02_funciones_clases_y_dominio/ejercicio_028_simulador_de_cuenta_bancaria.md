# Ejercicio 028. Simulador de cuenta bancaria
...
Construir una solucion funcional para `Simulador de cuenta bancaria` usando una estructura ordenada. El programa debe tener una entrada de datos, una capa de procesamiento y una salida verificable. Evita resolverlo todo dentro de un solo bloque.
...
Tu solucion debe modelar una entidad principal llamada `Movimiento` y una estructura que administre muchos elementos, por ejemplo `Libro financiero`. No te quedes en imprimir datos: el programa debe recibir, validar, transformar y mostrar informacion util.
...
- el saldo no debe quedar oculto tras errores.
...
Piensa si tu solucion necesita funciones o metodos parecidos a estos: `registrar_movimiento()`, `saldo_actual()`, `total_por_categoria()`, `resumen_mensual()`, `validar_monto()`. No copies estos nombres sin pensar; usalos como pista para descubrir responsabilidades.

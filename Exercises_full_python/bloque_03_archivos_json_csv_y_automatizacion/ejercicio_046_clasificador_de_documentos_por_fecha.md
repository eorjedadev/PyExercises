# Ejercicio 046. Clasificador de documentos por fecha
...
Construir una solucion funcional para `Clasificador de documentos por fecha` usando una estructura ordenada. El programa debe tener una entrada de datos, una capa de procesamiento y una salida verificable. Evita resolverlo todo dentro de un solo bloque.
...
Tu solucion debe modelar una entidad principal llamada `Documento` y una estructura que administre muchos elementos, por ejemplo `Conjunto de ejemplos`. No te quedes en imprimir datos: el programa debe recibir, validar, transformar y mostrar informacion util.

## Operaciones minimas

- preparar texto.
- extraer rasgos simples.
- clasificar.
- evaluar aciertos.
- explicar decision.
...
- separa datos de entrenamiento y prueba aunque sea manualmente.
- no ocultes cuando una regla falla.
- explica por que el sistema decidio una etiqueta.
...
Piensa si tu solucion necesita funciones o metodos parecidos a estos: `limpiar_texto()`, `extraer_palabras()`, `clasificar()`, `evaluar()`, `explicar_resultado()`. No copies estos nombres sin pensar; usalos como pista para descubrir responsabilidades.

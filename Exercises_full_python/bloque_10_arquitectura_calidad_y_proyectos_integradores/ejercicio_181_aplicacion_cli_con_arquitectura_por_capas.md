# Ejercicio 181. Aplicacion CLI con arquitectura por capas
...
Construir una solucion funcional para `Aplicacion CLI con arquitectura por capas` usando una estructura ordenada. El programa debe tener una entrada de datos, una capa de procesamiento y una salida verificable. Evita resolverlo todo dentro de un solo bloque.

## Area de entrenamiento

Arquitectura, calidad y proyectos integradores.

## Mision concreta

Tu solucion debe modelar una entidad principal llamada `Registro` y una estructura que administre muchos elementos, por ejemplo `repositorio de registros`. No te quedes en imprimir datos: el programa debe recibir, validar, transformar y mostrar informacion util.
...
- cada registro debe tener un identificador unico.
- los campos obligatorios no deben quedar vacios.
- las operaciones deben informar cuando algo no existe.
- la logica debe poder probarse sin depender de la interfaz.
...
Puedes apoyarte en: pytest, logging, dataclasses, typing, argparse, configparser, dotenv opcional, unittest.mock. No tienes que usarlas todas; el criterio esta en elegir las que realmente ordenan el problema.

## Funciones o metodos que deberias considerar

Piensa si tu solucion necesita funciones o metodos parecidos a estos: `agregar()`, `buscar()`, `actualizar()`, `listar()`, `resumir()`, `validar_configuracion()`, `registrar_log()`. No copies estos nombres sin pensar; usalos como pista para descubrir responsabilidades.
...
- crear un registro valido.
- intentar crear un registro duplicado.
- buscar un dato inexistente.
- actualizar un campo con valor invalido.
- prueba automatizada de una regla critica.
